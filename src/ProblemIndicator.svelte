<script>
   ...
   *   problem: { message: string, position?: { from: number, to: number } },
   *   onClick?: ((problem: any) => void) | null
   * } }
   */
  let { severity = 'error', problem, onClick = null } = $props();

  const interactive = $derived(onClick && problem && problem.position);

  const title = $derived((interactive ? 'Jump to ' : '') + severity + ': ' + problem.message);
